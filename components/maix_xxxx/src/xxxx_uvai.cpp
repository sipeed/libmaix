...
extern "C"
{
    ...
    void xxxx_vi_loop()
    {
        ...
        //     cv::rectangle(cv_src, cv::Point(24, 24), cv::Point(200, 200), cv::Scalar(0, 255, 0), 5);
    ...
    void maix_xxxx_main(int argc, char *argv[])
    {
        ...
        nn_test_app_load(&nn_test_app);
        while (xxxx->exit == 0)
        {
            xxxx_vi_loop();

            nn_test_app_loop(&nn_test_app);
            // xxxx_ui_loop();
            // xxxx_ai_loop();
        }
        nn_test_app_exit(&nn_test_app);
