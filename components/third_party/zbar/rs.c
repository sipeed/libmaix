...
int main(void){
  ...
  for(k=0;k<64*1024;k++){
    ...
    printk("%i %i",ndata,npar);
    for(i=0;i<ndata;i++)printk(" %i",data[i]);
    printk(" 0\n");
    ...
    if(npar>0){
      ...
      if(nerrors>0){
        ...
        for(i=0;i<ndata;i++)printk(" %i",data[i]);
        printk(" %i",nerrors);
        for(i=0;i<nerrors;i++)printk(" %i",epos[i]);
        printk("\n");
      }
    }
  }
  ...
  for(;;){
    ...
    if(nerrors>=0){
      ...
      for(i=ndata-npar;i<ndata;i++)if(data[i]!=data2[i]){
        printk("Abject failure! %i!=%i\n",data[i],data2[i]);
      }
      printk("Success!\n",nerrors);
      for(i=0;i<ndata;i++)printk("%i%s",data[i],i+1<ndata?" ":"\n");
    }
    else printk("Failure.\n");
  }
  return 0;
...
int main(void){
  ...
  for(a=0;a<256;a++)for(b=0;b<256;b++)for(c=0;c<256;c++)for(d=0;d<256;d++){
    ...
    if(mroots==4)for(j=0;j<mroots;j++){
      ...
      if(i>=nroots){
        printk("Missing root: (0x%02X)**4 ^ 0x%02X*(0x%02X)**3 ^ "
         "0x%02X*(0x%02X)**2 ^ 0x%02X(0x%02X) ^ 0x%02X = 0x%02X\n",
