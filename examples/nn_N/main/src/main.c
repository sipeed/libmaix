...
void nn_test(struct libmaix_disp *disp)
{
    ...
    if(! show)
    {
        LIBMAIX_DEBUG_PRINTF("create show image is failed\n");
        goto end;
    }
    ...
    while (!program_exit)
    {
        ...
        err = loc_decoder->decode(loc_decoder, loc_out_fmap, (void *)&loc_result);
        for (int i = 0; i < loc_result.num; ++i)
        {
            ...
            {
                int x1 = loc_result.plates[i].box.x * show->width;
                int y1 = loc_result.plates[i].box.y * show->height;
                int x2 = x1 + loc_result.plates[i].box.w * show->width ;
                int y2 = y1 + loc_result.plates[i].box.h * show->height;
                int w = loc_result.plates[i].box.w * show->width;
                ...
                // CALC_TIME_START();
                err = reg_decoder->decode(reg_decoder, &reg_out_fmap, (void *)&reg_result);
